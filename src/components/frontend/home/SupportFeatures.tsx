...
const SupportFeatures = () => {
  ...
  return (
    <div className="bg-olive-500 p-5 rounded-lg shadow-md text-white max-w-[380px] mx-auto space-y-3">
      {features.map((feature, index) => (
        <div key={index} className="flex items-start">
          ...
          <div className="flex-1 overflow-hidden">
            <h4 className="font-semibold text-sm mb-1 line-clamp-1">{feature.title}</h4>
            <p className="text-xs text-olive-100 line-clamp-2 leading-relaxed">{feature.description}</p>
          </div>
        </div>
      ))}
    </div>
